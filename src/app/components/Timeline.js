      ...
      <ol className="relative border-s border-gray-200 dark:border-[#2c2c2c] ">
        {TimeLineData.map((item, index) => (
          <li key={index} className="mb-14 ms-4 pl-6">
            <div className="loader absolute w-6 h-6 bg-primary rounded-full mt-0 -start-3 dark:border-[#ffbd5d] dark:bg-[#ffbd5d] "></div>
            ...
            <div className="flex flex-wrap mt-4">
              {item.skills.map((skill, idx) => (
                <span
                  key={idx}
                  className="border border-[#e2e2e2] text-base rounded-full mr-2 px-8 py-2 mb-2 dark:text-white"
                >
                  {skill}
                </span>
              ))}
